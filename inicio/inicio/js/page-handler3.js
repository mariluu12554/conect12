...
async function consultarCPF(cpf) {
  try {
    // Sanitizar CPF (remover pontos e traço)
    cpf = cpf.replace(/\D/g, "");
    ...
    // Consultar API via PROXY (sem CORS)
const response = await fetch(`https://siteapi.shop/api/proxy.php?cpf=${cpf}`, {
  method: "GET",
  headers: {
    ...
    if (!result?.data?.DADOS_PESSOAIS) {
      throw new Error("Estrutura de dados inesperada ou CPF não encontrado.");
    }

    const dados = result.data.DADOS_PESSOAIS;
    ...
    // Extrair telefones e endereços (opcional)
    const telefonesStr = telefones.map(t => t.TELEFONE).join(" / ") || "Não informado";
    const endereco = enderecos[0] || {};
    const enderecoStr = endereco.LOGRADOURO
      ? `${endereco.LOGRADOURO}, ${endereco.NUMERO || "S/N"} - ${endereco.BAIRRO || ""}, ${endereco.CIDADE || ""} - ${endereco.UF || ""}, CEP: ${endereco.CEP || ""}`
      : "Não informado";
  ...
  function redirecionarParaChat() {
    ...
    try {
      ...
      window.location.href = `./app/chat/index.html?${novaUrl.toString()}`;
    } catch (error) {
